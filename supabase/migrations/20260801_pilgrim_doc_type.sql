-- ════════════════════════════════════════════════════════════
-- تساهيل العمرة — Migration: نوع وثيقة المعتمر (هوية وطنية | إقامة | جواز سفر)
-- يضيف العمود doc_type لجداول المعتمرين ويحدّث دوال الحفظ لتخزينه.
-- آمن للتشغيل على قاعدة موجودة (بعد 20260729_customer_ux.sql).
-- الصقه في: Supabase → SQL Editor → Run.
-- ════════════════════════════════════════════════════════════

-- ═══════════════ 1) العمود الجديد ═══════════════
alter table booking_pilgrims add column if not exists doc_type text;
alter table payment_pilgrims add column if not exists doc_type text;
alter table ticket_pilgrims  add column if not exists doc_type text;

-- ═══════════════ 2) حفظ الحجز من لوحة الموظف ═══════════════
...
begin
  ...
  insert into booking_pilgrims(booking_id,name,doc_type,id_number,nationality,gender,birth_date,phone,sort)
    select v,e->>'name',nullif(e->>'docType',''),e->>'idNumber',e->>'nationality',e->>'gender',e->>'birthDate',e->>'phone',(o-1)::int
...
-- ═══════════════ 3) الحجز العام من صفحة المستفيد ═══════════════
...
-- ═══════════════ 4) استنتاج النوع للبيانات القديمة (مرة واحدة) ═══════════════
update booking_pilgrims set doc_type = case
  when id_number ~ '^1[0-9]{9}$' then 'national_id'
  when id_number ~ '^2[0-9]{9}$' then 'iqama'
  when id_number ~ '[A-Za-z]'    then 'passport'
end
where doc_type is null and coalesce(id_number,'') <> '';
